...
# ARRAY 0-349 SAM2BAM2BED READS ANNOTATION WITH BEDTOOLS INTERSECT
...
for file in bed_reads/*${name_mask}*res.bed; do
  /home/tools/bedtools/bedtools-v2.16.2/bin/intersectBed -a $file -b /home/dkhlebnikov/new_fRIP/reference/v39.basic.bed -wb > $file.intersected.bed;
done;

rm bed_reads/*${name_mask}*res.bed;
for file in bed_reads/*${name_mask}*intersected.bed; do
  awk '$4 == $8 {print $1"\t"$2"\t"$3"\t"$4"\t"$8"\t"$9"\t"$10}' $file > $file.tmp;
  mv $file.tmp $file;
done

for file in bed_reads/*${name_mask}*intersected.bed; do
  mv "$file" "${file/mapped_/}";
done

for file in bed_reads/*${name_mask}*intersected.bed; do
  mv "$file" "${file/_raw.sam.bam.bed.res.bed.intersected.bed/.bed}";
done

mkdir contacts
mv bed_reads/*${name_mask}*bed contacts/ 

for file in `ls contacts/*${name_mask}*bed`; do
  awk '{print $1"\t"$2"\t"$3"\t"$4"\t"$6"\t"$7}' $file > $file.tmp;
  mv $file.tmp $file;
done
